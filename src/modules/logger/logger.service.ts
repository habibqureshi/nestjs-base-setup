...
export class CustomLoggerService extends Logger {
  log(message: string) {
    ...
    super.log(`${requestId} ${user.email} ${timestamp} ${filename} : ${lineNumber}   ${message}`);
  ...
  error(message: string, trace: string, context?: string, request?: Request) {
    ...
    super.error(`[${timestamp}] [${filename}:${lineNumber}]  ${message}`, trace, context);
  ...
  private getFilename(): string {
    ...
    const filename = stack.substring(stack.lastIndexOf('/') + 1, stack.indexOf(':'));
    return filename;
  }
  ...
  private getLineNumber(): string {
    const error = new Error();
    const stack = error.stack?.split('\n')[3].trim() || '';
    const lineNumber = stack.substring(stack.lastIndexOf(':') + 1, stack.lastIndexOf(')'));
    return lineNumber;
  }
}
